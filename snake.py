...
def random_on_grid():       # gera a posição da maçã
...
while True:     #Não vai fechar a janela
    ...
    for i in range(len(snake_pos)-1, 0, -1):
        if collision(snake_pos[0], snake_pos[i]):
            restart_game()
            break
        snake_pos[i] = snake_pos[i-1]     # cada pedaço do corpo vai ocupar o espaço anterior
